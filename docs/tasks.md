...
- Objetivo de esta iteración: agregar soporte MVP para enlaces internos tipo anchor en Markdown y una acción simple para insertar una tabla de contenidos desde los headings del documento.
- Decisión técnica adoptada: centralizar slugs y extracción de headings en `src/lib/markdown/headingAnchors.ts`; reutilizar esos slugs en el render Markdown y en Exportar HTML mediante `renderMarkdown`.
- Alcance exacto: headings renderizados con `id` estable; links `#...` interceptados dentro del panel de preview con scroll al heading local; acción “Tabla de contenidos” en la toolbar; HTML standalone exportado con anchors funcionales; manual y changelog actualizados para que Novedades muestre `0.9.6`.
- Validación funcional cubierta por tests unitarios: slugify con español/acentos/signos/espacios, duplicados, fences, generación de tabla de contenidos, IDs en render Markdown y anchors en Exportar HTML sin UI interna de copiado.
...
- `pnpm test`: ejecutado correctamente; 11 archivos de test y 83 tests pasaron.
