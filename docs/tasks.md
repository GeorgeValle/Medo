...
## Estado general
En progreso: v0.6.8 ajuste pre-merge de borrador local con debounce + flush en eventos de ciclo de vida, manteniendo cierre nativo de ventana.
...
## Completadas
- Follow-up v0.6.8 pre-merge (esta iteración): persistencia de borrador local ajustada con debounce de 400ms para evitar escrituras síncronas por cada tecla, limpieza inmediata al volver a estado guardado, y `flush` final en `pagehide`/`beforeunload`/`visibilitychange` para maximizar recuperación sin interceptar ni bloquear el cierre nativo.
